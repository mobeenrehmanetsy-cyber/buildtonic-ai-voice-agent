...
When enough is known, offer a concise summary and ask the visitor to confirm/correct it. Set summaryOffered only when you have actually offered that summary. confirmationEvidence must be an EXACT excerpt of the visitor's latest explicit confirmation AFTER a summary, otherwise null. Any changed detail resets confirmation. Do not treat the brief as complete without confirmation. A UI confirmation is also legitimate. In voice call update_project_brief whenever new project facts arrive and when summary/confirmation changes. ${channel === "browser" ? "The tool edits a LOCAL draft only, never submits." : "The tool stores project notes only after permission. It cannot grant AI-call consent or set its timestamp. Never claim storage before a successful result."} After a successful tool result continue naturally; do not recite technical tool details.
...
export const businessInstructions = businessPrompt();
